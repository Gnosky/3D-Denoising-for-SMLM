...
if args.crop == 0:
    """
    train_imgs = []
    for i in range(np_train_imgs.shape[1]):
        if i + args.depth < np_train_imgs.shape[1]:
            new_img = np_train_imgs[0,i:i+args.depth,:,:,:]
            train_imgs.append(new_img)
    train_imgs = np.stack(train_imgs,axis=0)
    print("train_imgs.shape",train_imgs.shape)
    """
    def no_crop_generator(data, depth, batch_size):
        while True:
            z = np.random.randint(low=0, high=data.shape[1]-depth,size=batch_size)
            batch = np.zeros((batch_size,depth,512,512,1), dtype = data.dtype)
            for i, ind in enumerate(z):
                batch[i,:,:,:,:] = data[0,i:i+depth,:,:] 
            yield batch, None
    
    gen = no_crop_generator(np_train_imgs, args.depth, args.batch)
    val_gen = no_crop_generator(np_val_imgs, args.depth, args.batch)
    history = model.fit_generator(gen,
                        steps_per_epoch=np_train_imgs.shape[1]//args.batch,
                        validation_data = val_gen,
                        validation_steps = np_val_imgs.shape[1]//args.batch,
                        epochs=args.epoch, 
                        verbose=1,
                        callbacks=callbacks)
else:
